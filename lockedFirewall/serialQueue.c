...
int main(int argc, char * argv[]) {
  ...
  if(argc >= DEFAULT_NUMBER_OF_ARGS) {
    const int numPackets = 500000; // This is T. We just want a high value that we won't hit within 2 seconds.
    const int timeToRun = atoi(argv[1]); //This is M. Time in milliseoonds that experiment should run
    const int numSources = atoi(argv[2]); // This is n. equal to number of queues and workers
    const long mean = atol(argv[3]); // this is W. Expected work/packet.
    const int uniformFlag = atoi(argv[4]);
    const short experimentNumber = (short)atoi(argv[5]);


    firewall(numPackets,numSources,mean,uniformFlag,experimentNumber, timeToRun);
  }
  return 0;
}
		       ...
		       int timeToRun)
{
  ...
  workerData[i] = (workerData *)malloc(sizeof(workerData) + depth * sizeof(Packet_t *));
